...
from bokeh.plotting import figure, show, output_file, output_notebook, ColumnDataSource, curdoc
from bokeh.models import BoxSelectTool, LassoSelectTool, CustomJS, CheckboxGroup,\
                        LinearColorMapper, Slider, Panel, Select, AutocompleteInput, \
                        IndexFilter, CDSView, Div, Paragraph, Span, Button, RadioGroup
...
def make_ptvis_plot(data):
    ...
    def doing_ps(root,spot_index,eps,list_cluster,data_alpha,tissue_alpha):
        ...
        st.pl.trajectory.pseudotime_plot(data,list_cluster=list_cluster,
            show_graph=False,node_alpha=1,data_alpha=data_alpha,tissue_alpha=tissue_alpha,
            edge_alpha=0.1,node_size=3, show_plot=False, dpi=150,output="stlearn/static/",name="tmp"+str(id_img) )
        fig = Div(text="<img src='stlearn/static/tmp" + str(id_img) + ".png' height='482' width='568'>",sizing_mode='fixed')
    ...
    def change_ps():
        ...
        curdoc().add_next_tick_callback(doing_ps(root.value,spot_index.value,eps.value,
        list_cluster.active,data_alpha.value,tissue_alpha.value))
    ...
    def update_spot(attrname,old,new):
        ...
        spot_index = Slider(start=0, end=len_index, value=0, step=1, title="Spot index",sizing_mode='fixed')
    ...
    def create_figures(data,cluster,weight,reverse,tissue_alpha,data_alpha):
        ...
        st.spatial.trajectory.local_level(data,use_label="louvain",cluster=cluster,w=weight)
        ...
        st.pl.subcluster_plot(data,use_label="louvain",cluster=cluster,tissue_alpha=tissue_alpha,
            data_alpha=data_alpha,dpi=150,output="stlearn/static/",name="tmp"+str(id_img),show_plot=False)

        st.pl.trajectory.local_plot(data,use_cluster=cluster,branch_alpha=0.2,dpi=150,reverse=reverse_value,
            output="stlearn/static/",name="tmp"+str(id_img2),show_plot=False)

        fig = Div(text="<img src='stlearn/static/tmp" + str(id_img) + ".png' height='554' width='554' style='float: left;'>",sizing_mode='fixed')

        fig2 = Div(text="<img src='stlearn/static/tmp" + str(id_img2) + ".png' height='577' width='600' style='padding-left: 300px;'>",sizing_mode='fixed')
    ...
    def change_pst(attrname, old, new):
        ...
        fig,fig2 = create_figures(data,cluster_pst.value,weight_pst.value,reverse.active,
            tissue_alpha_pst.value,data_alpha_pst.value)
    ...
    def doing_pstg(list_cluster,weight,data_alpha,tissue_alpha):
        ...
        st.spatial.trajectory.global_level(data,use_label="louvain",list_cluster=list_cluster,w=weight)

        st.pl.cluster_plot(data,use_label="louvain",show_trajectory=True,list_cluster=list_cluster,show_plot=False,
            show_subcluster=True,show_legend=False, dpi=150,output="stlearn/static/",name="tmp"+str(id_img),
            data_alpha=data_alpha,tissue_alpha=tissue_alpha)

        st.pl.trajectory.tree_plot(data,dpi=150,output="stlearn/static/",name="tmp"+str(id_img2))

        fig = Div(text="<img src='stlearn/static/tmp" + str(id_img) + ".png' height='500' width='500' style='float: left;'>",sizing_mode='fixed')

        fig2 = Div(text="<img src='stlearn/static/tmp" + str(id_img2) + ".png' height='472' width='1200' style='padding-left: 300px;'>",sizing_mode='fixed')
    ...
    def change_pstg():
        ...
        curdoc().add_next_tick_callback(doing_pstg(list_cluster_pstg.active,weight_pstg.value,
            data_alpha_pstg.value,tissue_alpha_pstg.value))

    
    log_clustering = Paragraph(text="""""", width=0, height=0, sizing_mode='fixed')

    header = Div(text="""<h2>Step 4.2: Pseudotime analysis </h2>""",
    width=400, height=30, sizing_mode='fixed')

    data_alpha = Slider(title="Spot alpha", value=1.0, start=0, end=1.0, step=0.1,sizing_mode='fixed')
    ...
    eps = Slider(title="Eps (DBSCAN)", value=100, start=0, end=500, step=1,sizing_mode='fixed')
    ...
    inputs = column(header, root,spot_index, eps,p,list_cluster,data_alpha,tissue_alpha,ps_bt)
    ...
    data_alpha_pst = Slider(title="Spot alpha", value=1.0, start=0, end=1.0, step=0.1,sizing_mode='fixed')
    data_alpha_pst.on_change('value',change_pst)

    tissue_alpha_pst = Slider(title="Tissue alpha", value=1.0, start=0, end=1.0, step=0.1,sizing_mode='fixed')
    tissue_alpha_pst.on_change('value',change_pst)

    

    pst_layout = column([header_pst,cluster_pst,weight_pst,reverse,data_alpha_pst,tissue_alpha_pst])

    figures = row(create_figures(data,cluster_pst.value,weight_pst.value,reverse.active,
            tissue_alpha_pst.value,data_alpha_pst.value))
    ...
    header_pstg = Div(text="""<h2>Pseudo-space-time. Global analysis </h2>""",
    width=400, height=30, sizing_mode='fixed')
    ...
    data_alpha_pstg = Slider(title="Spot alpha", value=1.0, start=0, end=1.0, step=0.1,sizing_mode='fixed')
    #data_alpha_pstg.on_change('value',change_pstg)

    tissue_alpha_pstg = Slider(title="Tissue alpha", value=1.0, start=0, end=1.0, step=0.1,sizing_mode='fixed')
    #tissue_alpha_pstg.on_change('value',change_pstg)

    ps_bt = Button(label='Do global pseudo-space-time analysis',button_type="success", sizing_mode='fixed')
    ps_bt.on_click(change_pstg)

    pstg_layout = column([header_pstg,p,list_cluster_pstg,weight_pstg,data_alpha_pstg,tissue_alpha_pstg,ps_bt])
