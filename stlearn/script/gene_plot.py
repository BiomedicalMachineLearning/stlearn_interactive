...
from bokeh.models import BoxSelectTool, LassoSelectTool, CustomJS, Div,\
                        LinearColorMapper, Slider, Panel, Select, AutocompleteInput, \
                        ColorBar, TextInput,  BasicTicker, CrosshairTool,HoverTool
...
def make_gene_plot(data):
    ...
    image = (data.uns["spatial"][library_id]["images"][data.uns["spatial"]["use_quality"]]* 255).astype(np.uint8)
    ...
    def make_fig():
        ...
        fig = figure(title="Gene plot",
                     x_range=(0,dim-150), y_range=(dim,0),
                     )
        ...
        fig.image_rgba(image=[img], x=0, y=xdim, dw=ydim, dh=xdim,global_alpha=tissue_alpha.value)

        
        color_mapper = LinearColorMapper(palette=Spectral11, low=min(s1.data['color']), high=max(s1.data['color']))

        fig.circle("x","y",
                 color={'field': 'color', 'transform': color_mapper}, size=5, source=s1,
                  fill_alpha =data_alpha.value, line_alpha=data_alpha.value)
    ...
    header = Div(text="""<h2>Gene plot: </h2>""",
    ...
    gene_select = AutocompleteInput(title="Gene:", value=gene_list[0], completions=gene_list,
        min_characters=1)
