...
from bokeh.models import TextInput, Button, Paragraph, Select, Div, \
                        CheckboxGroup, Tabs, Panel, AutocompleteInput, \
                        RadioGroup, Slider
...
##################### Tab 1 #############################
header_step1 = Div(text="""<h2>Step 1: Reading data </h2>""",
width=400, height=50)

header_step2 = Div(text="""<h2>Step 2: Preprocessing </h2>""",
width=400, height=50)
...
preprocessing = CheckboxGroup(
        labels=["Filter genes", "Normalize total", "Log 1P",
                "Scale"], active=[0, 1, 2, 3])

##################### Tab 3 #############################

# Setup setting for clustering
header_step3 = Div(text="""<h2>Step 3: SME Clustering </h2>""",
width=400, height=50, sizing_mode='fixed')



pca_slider = Slider(title="PCA - Number of component", value=50, start=1, end=100, step=1, sizing_mode='fixed')

sme_clust = RadioGroup(
        labels=["Using SMEClust", "Don't use"], active=0, sizing_mode='fixed')

tiling_slider = Slider(title="Crop size for tiling image", value=40, start=1, end=100, step=1, sizing_mode='fixed')

cl_method = Select(title="Choosing clustering method:", 
    value="Louvain clustering", 
    options=["Louvain clustering", "K-means clustering"], sizing_mode='fixed')

# Setup for louvain clustering

knn_slider = Slider(title="Number of neighbors for KNN", value=25, start=2, end=100, step=1, sizing_mode='fixed')

morpho_radius = Slider(title="Radius for adjusting spots", value=50, start=2, end=100, step=1, sizing_mode='fixed')

# Setup for k-means clustering

n_clusters = Slider(title="Number of clusters", value=10, start=2, end=100, step=1, sizing_mode='fixed')
...
header_step4 = Div(text="""<h2>Step 4.1: Pseudotime analysis </h2>""",
width=400, height=50, sizing_mode='fixed')

##################### Tab 5 #############################

# Setup setting for pseudotime visualization
header_step5 = Div(text="""<h2>Step 4.2: Pseudotime analysis ST visualization </h2>""",
width=400, height=50, sizing_mode='fixed')
...
def doing_smeclust():
    ...
    if sme_clust.active == 0:
        ...
        st.spatial.morphology.adjust(data,use_data="X_pca",radius=morpho_radius.value,method="mean")
...
tab3_layout = [header_step3,pca_slider,sme_clust,tiling_slider,morpho_radius,
                cl_method,knn_slider,sme_bt,log_clustering]
